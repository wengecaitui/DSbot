...
def _trend_impulse_payload():
    p = {"label":"NEW DERIVED STRATEGY SPEC","strategyId":_SID,"version":"1.0.0",
...
class RuleSpecTests(unittest.TestCase):
    ...
    def test_unsupported_execution_rejected(self):
        payload = dict(_trend_impulse_payload()); payload["executionTiming"]="intrabar"
        with self.assertRaises(ValueError):
...
class SnapshotTests(unittest.TestCase):
    ...
    def test_snapshot_no_output_rejects_components(self):
        ...
        with self.assertRaises(ValueError):
            create_component_snapshot(strategy_id=_SID,spec_id=spec.spec_id,
                ...
                component_outputs={"TrendImpulse":{"signal":"BULL"}},component_ids=spec.components)
    ...
    def test_snapshot_has_output_rejects_empty(self):
        ...
        with self.assertRaises(ValueError):
            create_component_snapshot(strategy_id=_SID,spec_id=spec.spec_id,
                parameter_id=spec.parameter_id,dataset_id=_DID,symbol=_SYM,
                bar_open_time_ms=0,has_outputs=True,component_outputs={},component_ids=spec.components)
...
class ProducerTests(unittest.TestCase):
    ...
    def test_component_field_missing(self):
        spec=_trend_spec()
        s=_snap(spec,0,components={"TrendImpulse":{"name":"TI"}})
        with self.assertRaises(ValueError):
            self._produce(spec,(s,))
    ...
    def test_final_bar_reserved(self):
        with self.assertRaises(ValueError):
            self._produce(_trend_spec(),tuple(_snap(_trend_spec(),i*F,components={"TrendImpulse":{"signal":"BULL"}}) for i in range(10)),scored_end=F*10)
    ...
    def test_snapshot_time_not_aligned(self):
        spec=_trend_spec()
        with self.assertRaises(ValueError):
            create_component_snapshot(strategy_id=_SID,spec_id=spec.spec_id,parameter_id=spec.parameter_id,
                dataset_id=_DID,symbol=_SYM,bar_open_time_ms=150_000,has_outputs=False,
    ...
    def test_verify_rejects_mismatch(self):
        ...
        batch=self._produce(spec,snaps)
        with self.assertRaises(ValueError):
            verify_observation_batch(batch=batch,spec=spec,snapshots=snaps,dataset_id="x"*64,
    ...
    def test_source_digest_missing_component_rejected(self):
        payload=_trend_impulse_payload(); payload["sourceAssetDigests"]={}
        with self.assertRaises(ValueError):
            create_frozen_rule_spec(payload,_spec_id(payload),{"tp":21,"tm":2.0,"max_holding_bars":96})

    def test_numeric_string_gte_rejected(self):
        # Use custom spec with gte rule that hits a string field value
        payload = dict(_trend_impulse_payload())
        payload["exitRules"]=[{"position":"long","any":[{"component":"TrendImpulse","field":"signal","operator":"gte","value":0}]}]
        spec_id = canonical_sha256(payload)
        spec = create_frozen_rule_spec(payload, spec_id, {"tp":21,"tm":2.0,"max_holding_bars":96})
        # Signal "BULL" is a string — gte should fail on non-numeric
        s = _snap(spec, 0, components={"TrendImpulse":{"signal":"BULL"}})
        with self.assertRaises(ValueError):
            self._produce(spec, (s,))
    ...
    def test_duplicate_components_rejected(self):
        payload=_trend_impulse_payload()
        payload["components"]=[{"assetId":"TrendImpulse","parameterMap":{"period":"tp"}},{"assetId":"TrendImpulse","parameterMap":{"period":"tp2"}}]
        with self.assertRaises(ValueError):
            create_frozen_rule_spec(payload,_spec_id(payload),{"tp":21,"tp2":2.0,"max_holding_bars":96})
    ...
    def test_duplicate_snapshot_ids_rejected(self):
        spec=_trend_spec(); s=_snap(spec,0,components={"TrendImpulse":{"signal":"BULL"}})
        with self.assertRaises(ValueError):
            self._produce(spec,(s,s))

    def test_missing_field_in_outputs_true_rejected(self):
        spec=_trend_spec()
        with self.assertRaises(ValueError):
            self._produce(spec,(_snap(spec,0,components={"TrendImpulse":{}}),))
